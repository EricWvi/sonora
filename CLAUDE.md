...
- Crate names are prefixed with `sonora-`. For example, the `core` folder's crate is named `sonora-core`
...
- Use sonora-logging wrapper macros instead of `tracing` macros. Use `sonora_logging::clock::now_local` instead of `OffsetDateTime::now_local()`.
