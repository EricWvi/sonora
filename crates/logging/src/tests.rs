...
fn formats_json_events_with_context_and_error_objects() {
    ...
    assert_eq!(
        stdout_event["target"],
        Value::String("sonora_logging::tests".to_string())
    );
    assert_eq!(
...
fn emits_method_field_from_wrapper_macros() {
    ...
    with_default(&dispatch, || {
        crate::sonora_info!(message = "macro event");
    });
...
fn selects_file_only_sink_behavior() {
    ...
    assert_eq!(
        read_rotated_log_lines(temp_dir.path(), "sonora.log").len(),
        1
    );
}
...
fn cleans_up_rotated_files_by_retention_window() {
    let temp_dir = TempDir::new().unwrap();
    create_log_file(temp_dir.path(), "sonora.log.2026-05-01");
    create_log_file(temp_dir.path(), "sonora.log.2026-05-02");
    create_log_file(temp_dir.path(), "sonora.log.2026-05-03");
    create_log_file(temp_dir.path(), "sonora.log.2026-05-04");
    create_log_file(temp_dir.path(), "unrelated.log.2026-05-01");

    cleanup_old_logs(
        &ActiveLogPath::from_path(&temp_dir.path().join("sonora.log")).unwrap(),
        3,
    )
    .unwrap();
    ...
    assert_eq!(
        ...
        vec![
            "sonora.log.2026-05-02".to_string(),
            "sonora.log.2026-05-03".to_string(),
            "sonora.log.2026-05-04".to_string(),
